...
public partial class AgentServer
{
    ...
    private readonly ConcurrentDictionary<Guid, ConcurrentQueue<Messages.Message>> _socketMessageSendingQueue = new();
    ...
    public void Publish(Messages.Message message, string? token = null)
    {
        try
        {
                ...
                try
                {
                    if (token is null || (_socketTokens.TryGetValue(connectionId, out string? val) && val == token))
                    {
                        if (_socketMessageSendingQueue.TryGetValue(
                            connectionId, out ConcurrentQueue<Messages.Message>? queue
                            ) && queue is not null)
                        {
                        ...
                        else
                        {
                            _socketMessageSendingQueue.AddOrUpdate(
                                connectionId,
                                new ConcurrentQueue<Messages.Message>(),
                                (key, oldValue) => new ConcurrentQueue<Messages.Message>()
                            );
    ...
    private Task CreateTaskForSendingMessage(Guid socketId)
    {
        ...
        return new(() =>
        {
            while (_isRunning)
            {
                ...
                try
                {
                    if (_socketMessageSendingQueue.TryGetValue(socketId, out ConcurrentQueue<Messages.Message>? queue))
                    {
                        ...
                        if (queue.TryDequeue(out Messages.Message? message) && message is not null)
                        {
